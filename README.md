...
- 2024년 2학년 2학기 인공지능 응용 수업을 들으며 배운 내용들을 정리하여 작성하였습니다.
  <br><br>
# 주요 내용<br><br>
- [LLM 예제 실습](#2-llm-실습-및-할루시네이션-문제-해결)
- [할루시네이션 극복 방법](#할루시네이션-극복-방법)
...
대형 언어 모델(LLM)은 주어진 입력 데이터를 바탕으로 예측을 수행하고, 해당 데이터를 기반으로 학습된 방대한 텍스트 데이터에서 적절한 답변을 생성합니다. 모델은 일반적으로 다음 단계를 통해 작동합니다:

1. **입력 처리**:  
   - 사용자가 제공한 텍스트를 토큰화하여 모델이 처리 가능한 숫자형 데이터로 변환합니다.
   - 예: `"What is AI?"` → `[101, 2054, 2003, 4569, 102]`

2. **모델 연산**:  
   - 토큰화된 데이터를 모델의 신경망에 전달하여 확률 분포를 계산합니다.
   - 예: 다음 단어로 적합한 후보군(`AI`, `robotics`, `technology` 등)의 확률을 평가.

3. **출력 생성**:  
   - 계산된 확률에 따라 최적의 응답을 구성하여 사용자에게 제공.
   - 예: `"AI is the simulation of human intelligence by machines."`
...
1. **토큰화(Tokenization)**:  
   텍스트 데이터를 이해하기 위해 단어, 부분 단어, 또는 문자 단위로 나누는 과정을 수행합니다. 이를 통해 입력 텍스트는 숫자 형태의 벡터로 변환됩니다.

- **토큰라이저 기반 처리**:  
텍스트는 모델 내부에서 다음과 같이 분해됩니다:
- 입력: `"Translate the following English sentence to French: 'How are you today?'"`  
- 토큰화: `[20034, 3452, 2034, ...]`
- 모델 연산 후 출력: `"Comment allez-vous aujourd'hui ?"`.

LLM은 이러한 과정을 반복적으로 수행하여 고도로 정확한 텍스트 생성을 가능하게 합니다.

2. **프롬프트 입력**:  
   사용자는 특정 질문이나 지시사항을 모델에 입력(프롬프트)으로 제공합니다. 이 프롬프트는 텍스트 형식으로 작성되며, 모델이 이를 기반으로 답변을 생성합니다.

3. **예측 및 출력 생성**:  
   LLM은 주어진 프롬프트를 기반으로 적합한 다음 단어 또는 문장을 예측하여 응답을 생성합니다. 이 과정은 확률 기반으로 이루어지며, 모델이 가장 적합하다고 판단한 출력이 최종적으로 제공됩니다.
